...
base_path = 'G:\MPI-FAUST\training\scans\';
...
dire = dir(fullfile(base_path, '*.ply'));
...
for i = 1:length(dire)
    display(dire(i).name);
    [vertex,face] = read_ply(fullfile(base_path,[dire(i).name]));
    saveoff(vertex,face,fullfile(base_path,[int2str(i-1),'.off']));
    G = Mesh(params.preprocess.dataFileType,fullfile(base_path,[int2str(i-1),'.off']));
    node = G.V;
