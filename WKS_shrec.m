clear;close all;
dir = 'G:\SHREC14\Real\Data\';
for ind = 0:399
    %[vertices, faces] = read_m2('H:\ADNI2_01_20\CTL_processed\006_S_4449\lh_pial_final.m');
    [vertices, faces] = readoff([dir,num2str(ind),'.off']);
    %vertices = vertices';
    %faces = faces';
    n_eigenvalues = getoptions([],'Nev',300); % number of eigenvalues used for computations
    % depending on the application, you can use less than 300
    N = getoptions([],'N',128); % number of evaluations of WKS
    wks_variance = getoptions([],'wks_variance',6); % variance of the WKS gaussian (wih respect to the
    ...
    save([dir,num2str(ind),'_WKS_128.txt'],'WKS','-ascii');
end
%plotMesh0(vertices,faces,WKS(:,2));