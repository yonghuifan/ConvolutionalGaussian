...
data_wks = [];
count = 1;
num_points = 120;%30  50  80  120  200 300
...
for class = 1: 10
    ...
    for i = (class-1):10:399
        ...
        data_wks = load(fullfile(base_path,[int2str(i),'_WKS_64.txt']));
        % acc_cur_mark = zeros(7,length(1:10:500));
        % fan_sihks_tmp = [fan_sihks_tmp; reshape(pca(data_sihks(fan_lmk,:)),1,[])];
        % fan_wks_tmp = [fan_wks_tmp; reshape(pca(data_wks(fan_lmk,:)),1,[])];
    ...
    save(fullfile(base_path,['fan_category_',int2str(class),'_',int2str(num_points),'_wks_64.mat']),'fan_wks_tmp');
