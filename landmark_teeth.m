...
base_path = 'G:\CPsurfcomp\DATA\teeth\meshes\';
...
%     tmp(Lmk_needed+1:end) = [];
    tmp = Lmks;
    %save([fullfile(base_path,[dire(i).name]),'_Fan3_landmark600.txt'],'Lmks','-ascii');
    ...
    axis equal
    %set(gca, 'Layer', 'top')
    pp= patch('Faces',face,'Vertices',node,'FaceVertexCData',abs(GC*600),'FaceColor','interp','EdgeColor','none','BackFaceLighting','lit');
